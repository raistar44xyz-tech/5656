...
def check_cookie(cookie_text: str, generate_token: bool = True, bulk_mode: bool = False) -> dict:
    ...
    # Generous timeouts — Netflix needs breathing room, especially under concurrent load.
    # Proxy gets the same time as direct so the race is fair; dead proxies lose fast
    # because their TCP handshake fails immediately (no timeout needed to detect them).
    _proxy_timeout  = 5  if bulk_mode else 8    # seconds per proxy attempt
    _direct_timeout = 10 if bulk_mode else 14   # direct gets more time — most reliable
    _timeout = _direct_timeout                  # used by NFToken
    session, using_curl = _make_session(bulk_mode)
    session.cookies.clear()   # flush response-cookies from the previous check
    ...
    def _fetch_account():
        """
        Smart fetch — always as fast as direct, never slower, even with dead proxies.

        Strategy: race ONE round-robin proxy vs a direct connection simultaneously.
        • Working proxy  → proxy wins (different IP, speed = proxy latency)
        • Dead/slow proxy → direct wins (started at same time, no extra wait)
        • No proxy       → direct only, generous timeout

        Single rotating proxy per check (not race-N) keeps connection count low
        and ensures true rotation across the full pool.
        """
        ...
        def _one_attempt(sess, proxy_dict, url):
            """
            Single HTTP request using *sess* (caller-owned session object).
            Each racing thread gets its own session → no cross-thread state sharing.
            Returns ("OK"|"INVALID"|"LOGIN"|"FAIL", html).
            """
            ...
            try:
                ...
                if _is_rate_limited(r.status_code, r.text):
                    if proxy_dict and _proxy_manager:
                        _proxy_manager.mark_failure(_purl(proxy_dict), hard=True)
                    return "FAIL", ""
                if r.status_code in (401, 403):
                    return "INVALID", r.text
                if _is_login_page(r.url, r.text):
                    if proxy_dict and _proxy_manager:
                        # Hard-remove: proxy serving login pages is blocked/useless
                        _proxy_manager.mark_failure(_purl(proxy_dict), hard=True)
                    return "LOGIN", r.text
                if r.status_code == 200 and len(r.text) > 1000:
                    if proxy_dict and _proxy_manager:
                        _proxy_manager.mark_success(_purl(proxy_dict), latency=_elapsed)
                    return "OK", r.text
                return "FAIL", ""
            except Exception as exc:
                logger.debug("_one_attempt error proxy=%s: %s", bool(proxy_dict), exc)
                if proxy_dict and _proxy_manager:
                    _proxy_manager.mark_failure(_purl(proxy_dict), hard=True)
                return "FAIL", ""
        ...
        def _smart_check(url):
            """
            Proxy ON  → race 3 proxies. Direct NEVER used. Server IP stays hidden.
            Proxy OFF → direct only, generous timeout.

            Session safety: each proxy worker calls _make_session() from its OWN
            thread → gets its own thread-local session → zero cross-thread sharing.

            Verdict rules (all source-aware):
              ...
              LOGIN   from 1 proxy        → trust after all others finish/fail
              FAIL    from all            → caller tries next URL or retries
            """
            # ── Direct-only (proxy OFF) ───────────────────────────────────────
            ...
            # ── Proxy-only (proxy ON) — never touch server IP ─────────────────
            RACE_N  = 3
        ...
        # ── Main fetch — 3-layer error elimination ────────────────────────────
        #
        # Layer 1: /account/membership   (richest data, primary target)
        # Layer 2: /browse               (different URL — bypasses some proxy blocks)
        # Layer 3: retry /membership after 0.5s pause (proxies rotate, fresh pick)
        #          → single mode only; bulk skips this to keep speed acceptable
        #
        # ERROR is only returned if ALL layers fail, which in practice means
        # the proxy pool is empty or Replit's network is down.

        s, h = _smart_check("https://www.netflix.com/account/membership")
        if s in ("OK", "INVALID"):
            return s, h

        s, h = _smart_check("https://www.netflix.com/browse")
        if s in ("OK", "INVALID"):
            return s, h
